...
#############################################################################################################
st.subheader('prediction of next 10 days price')
i=0
from sklearn.preprocessing import MinMaxScaler
j=len(df['Close'])-100
scaler=MinMaxScaler(feature_range=(0,1))
...
# print(test)
# print()
past=test.reshape(1,-1)
# print(past)
past_data=list(past)
past_data=past_data[0].tolist()
# print(temp_input)
# print(past)
# print()
# print(test)
...
while(i<10):
    if(len(past_data)>100):
        past=np.array(past_data[1:])
        # st.text(f'{i} day predicted price {past_data}')
        past=past.reshape(1,-1)
        past = past.reshape((1,100,1))
        y_pre = model.predict(past,verbose=0)
        # st.text(f'{i} day predicted {y_pre}')
        past_data.extend(y_pre[0].tolist())
        past_data = past_data[1:]
        output.extend(y_pre.tolist())
        i=i+1
    else:
        past=past.reshape((1,100,1))
        y_pre = model.predict(past,verbose=0)
        ...
        output.extend(y_pre.tolist())
        i=i+1
real=scaler.inverse_transform(output)
for i in range(len(real)):
    st.text(f'{i+1} day predicted {real[i]}')         
st.text(f'all the predicted output are {real}')
pre=np.arange(1,31)
st.subheader('graph for next 10 days')
fig_z = plt.figure(figsize=(12,10))
plt.plot(real)
plt.xlabel('Days')
plt.ylabel('Price')
st.pyplot(fig_z)
lst=df['Close'].tolist()
lst.extend(real)
st.subheader('overall graph along with the predicted')
fig_zzz = plt.figure(figsize=(12,10))
plt.plot(lst)
plt.xlabel('Days')
plt.ylabel('Price')
st.pyplot(fig_zzz)
